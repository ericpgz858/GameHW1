...
namespace Platformer.Mechanics
{
    ...
    /// </summary>
    public class ArcherController : KinematicObject
    {
        ...
        {
            return Physics2D.Raycast(playerCheck.position, transform.right, AttackDistance, whatIsPlayer);
        }
        public virtual bool CheckPlayerInBack()
        {
            if (Vector2.Distance(Player_T.position, playerCheck.position) <= playerCheckDistance && (Player_T.position.x - playerCheck.position.x) * FacingDirection < 0)
            {
                return true;
            }
            return false;
        ...
        private void Attack()
        {
            GameObject ArrowInc = Instantiate(Arrow, Fireposition.position, transform.rotation);
            ArrowInc.GetComponent<Rigidbody2D>().AddForce(transform.right * LaunchForce);
        }
    }
}